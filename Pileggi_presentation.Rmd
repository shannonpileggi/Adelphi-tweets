...
* Explore R packages
  ...
  + Continued learning: `tidyverse`, `skimr`, `lubridate`, `broom`, `flexdashboard`, `DT`
...
* tweets begin at `r ar_tweets$created_at[nrow(ar_tweets)]`,  
end at `r ar_tweets$created_at[1]`
...
### Common hashtag themes {data-commentary-width=400}
...
### Trends with hashtag themes {data-commentary-width=650}
```{r}
ar_hashtags_year %>% 
...
ar_hashtags %>% 
  ...
  ggplot(aes(x = hashtags_theme, 
             y = count, 
             fill = popularity)) +
             # fill = fct_reorder(popularity, count, .desc = T))) +
    geom_violin() +
    coord_flip() +
...
### Hashtag network {data-commentary-width=650}
```{r}
...
top_corrs_ht %>% 
  graph_from_data_frame(vertices = vertices_ht) %>% 
    ...
    geom_node_point(aes(size = occurrences,
                        color = geom_mean_likes)) + 
    geom_node_text(aes(label = name), repel = TRUE) + 
    theme_void() +
    labs(color = "Average likes",
         size = "Frequency",
         title = "Frequency, likes, and relationships among @AdelphiResearch hashtags",
    ...
    # scale_color_gradient(low='#E1FA72', high='#5011D1')
...
### Text network {data-commentary-width=650}
...
top_corrs_text %>% 
  graph_from_data_frame(vertices = vertices_text) %>% 
    ...
    labs(color = "Average likes",
         size = "Frequency",
         title = "Frequency, likes, and relationships among top 100 words") +
    scale_color_gradient(low='#E1FA72', high='#F46FEE')
...
tidy(lasso_model$glmnet.fit) %>%
  ...
  theme_minimal() +
    ...
    labs(y = "Estimate", 
         ...
         subtitle = "Terms with at least 30 occurrences") +
    scale_fill_discrete(name = "Association",
                      breaks = c(FALSE, TRUE),
                      labels = c("Positive", "Negative"))
...
* `#mrx` / `#marketresearch` occurs frequently and often appears with other hashtags
* `#innovation` tends to get more likes; `#mrs` and `#bhbia` tends to get more retweets

#### Tweet text
* Network analysis: words that got together and tend to get more likes/retweets 
...
* Model analysis: 
  + words most positively associated with retweets/likes: conference, team
  + words most negatively associated with retweets/likes: UK, guardian
...
* Incorporate other variables (text length, whether or not has a url, whether or not has a picture, time, etc.) in a unified model.
* Multivariate modeling with retweets and likes as a dual outcome?
* Build an app that lets users predict number of retweets/likes based on input words or hashtags.
